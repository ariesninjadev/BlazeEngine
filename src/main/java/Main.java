...
import com.ariesninja.BlazeEngine.gui.DepthMapVisualizer;
import com.ariesninja.BlazeEngine.math.Attenuation;
import com.ariesninja.BlazeEngine.math.Shadows;
...
// Start a new thread to update the depth map visualization
        new Thread(() -> {
            while (true) {
                try {
                    Thread.sleep(1000); // Update every 100 milliseconds
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
                double[][] depthMap = Shadows.createDepthMap(client, light2);
